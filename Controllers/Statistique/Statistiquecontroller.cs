...
namespace scoring_Backend.Controllers.Statistique
{
    ...
    public class StatistiqueController : ControllerBase
    {
        ...
        private int GetUserRole() => int.Parse(User.FindFirst("role")?.Value ?? "0");
        private int GetSiteId()   => int.Parse(User.FindFirst("siteId")?.Value ?? "0");
    ...
    catch (Exception ex)
    {
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            try
            {
                ...
                {
                    "section-stats"     => await _statistiqueRepository.GetSectionStatsAsync(request.Filter, GetUserId(), GetUserRole(), GetSiteId()),
                    "agent-scores"      => await _statistiqueRepository.GetAgentScoresAsync(request.Filter, GetUserId(), GetUserRole(), GetSiteId(), request.SortDirection),
                    "program-level"     => await _statistiqueRepository.GetProgramLevelAsync(request.Filter, GetUserId(), GetUserRole(), GetSiteId(), request.AllSupervisors),
                    "coaching-sheet"    => await _statistiqueRepository.GetCoachingSheetAsync(request.Filter, request.AgentId ?? 0, GetUserId(), GetUserRole(), request.AllSupervisors),
                    "coaching-analysis" => await _statistiqueRepository.GetCoachingAnalysisAsync(request.Filter, request.AgentId ?? 0, GetUserId(), GetUserRole(), request.AllSupervisors),
                    "coaching-summary"  => await _statistiqueRepository.GetCoachingSummaryAsync(request.Filter, request.AgentId ?? 0, GetUserId(), GetUserRole(), request.AllSupervisors),
                    _ => null
                };

                if (data == null) return BadRequest("Unknown report type");

                var (bytes, contentType, fileName) = request.Format.ToUpper() switch
                {
                    "PDF"  => (ExportHelper.ToPdf(data, request.ReportType, request.Filter),  "application/pdf",  $"{request.ReportType}.pdf"),
                    "XLS"  => (ExportHelper.ToXls(data),                                       "application/vnd.ms-excel", $"{request.ReportType}.xls"),
                    "CSV"  => (ExportHelper.ToCsv(data),                                       "text/csv",         $"{request.ReportType}.csv"),
                    "RTF"  => (ExportHelper.ToRtf(data, request.ReportType, request.Filter),   "application/rtf",  $"{request.ReportType}.rtf"),
