...
namespace scoring_Backend.Repositories.Implementations.Evaluation
{
    public class EvaluationListRepository : IEvaluationListRepository
    {
        ...
        // ─────────────────────────────────────────────────────
        // 1. Liste paginée des fiches d'écoute (Ls)
        //    ✅ Amélioration : Étape 6 supprimée (CalledCampaignId
        //       déjà présent dans le SELECT de l'Étape 4, colonne 10)
        //    ✅ Amélioration : roleFilter utilise des sous-requêtes
        //       paramétrées au lieu d'interpoler userId/siteId
        // ─────────────────────────────────────────────────────
    ...
    EvaluationListFilterDto filter)
{
    ...
    // ✅ On récupère les SURVEY IDs, pas les LsIds
        ...
        foreach (var cf in filter.ColumnFilters)
        {
            switch (cf.Id)
            {
                ...
                case "score" when double.TryParse(cf.Value, out var sc):
                    conditions.Add($"s.Score >= {sc}");  // ✅ score de la survey
                    break;
            }
        }
   ...
   // ── Étape 3 : Total — nombre d'enregistrements distincts (groupés) ──
int total = 0;
await using (var cmdTotal = conn.CreateCommand())
{
    ...
    SELECT
        ...
        l.StartPeriode,
        l.EndPeriode,
        l.CreateDate       AS LsCreateDate,
        l.UpdateDate       AS LsUpdateDate,
        l.CalledCampaignId,
        -- ✅ Prendre le dernier surveyId pour ce recordDataId
        MAX(s.Id)          AS SurveyId,
        -- ✅ Score moyen ou score du dernier survey
        AVG(CAST(s.Score AS FLOAT)) AS SurveyScore,
        MAX(s.CreateDate)  AS SurveyDate,
        s.RecordDataId
    FROM Ls_survey s
    JOIN Ls l ON l.Id = s.LsId
    WHERE s.Id IN ({surveyList})
    {roleFilter}
    -- ✅ Grouper par recordDataId pour avoir une ligne par enregistrement
    GROUP BY
        l.Id, l.Agent, l.AgentOid, l.AgentId, l.Auditor,
        l.StartPeriode, l.EndPeriode, l.CreateDate, l.UpdateDate,
        l.CalledCampaignId, s.RecordDataId
            ...
            items.Add(new LsFicheDto
{
    ...
    // ✅ FIX : AVG retourne double, pas float — utiliser Convert.ToDouble
    Score        = reader.IsDBNull(11) ? 0
                 : Math.Round(Convert.ToDouble(reader.GetValue(11)), 2),
    ...
    return (items, total);
        ...
        // ─────────────────────────────────────────────────────
        // 2. Surveys d'une fiche (aucun changement nécessaire)
        // ─────────────────────────────────────────────────────
        ...
        // ─────────────────────────────────────────────────────
        // 3. Items d'une survey (aucun changement)
        // ─────────────────────────────────────────────────────
            ...
            int surveyId, int auditorId, UpdateSurveyDto dto)
        {
            ...
            Console.WriteLine($"[DEBUG] surveyId={surveyId} | site={site} | newScore={newScore}");
    ...
    : await GetScalarAsync("SELECT [dbo].[Fn_Ls_getLsScore](@p0, 0)",  lsId);
            ...
            // ✅ APRÈS — on utilise newScore qu'on vient de calculer
//           + on enrichit avec catégorie et motif appel
...
await using (var cmd = conn.CreateCommand())
{
    cmd.CommandText = @"
        SELECT s.Id, s.LsId, s.CreateDate, s.UpdateDate,
               ...
               rd.Record_Date                          -- ✅ ajout
        FROM Ls_survey s
        LEFT JOIN Ls_categories cat ON cat.Id = s.Id_Categories
        LEFT JOIN Ls_CallReason cr  ON cr.Id  = s.Id_CallReason
        LEFT JOIN RecordData    rd  ON rd.ID  = s.RecordDataId  -- ✅ ajout
        WHERE s.Id = @id";
    AddParam(cmd, "@id", surveyId);
    await using var reader = await cmd.ExecuteReaderAsync();
    if (await reader.ReadAsync())
    {
        updated = new LsSurveyDto
        {
            ...
            RecordDate      = reader.IsDBNull(10) ? ""  // ✅ ajout
        ...
        public async Task DeleteLsFicheAsync(int lsId)
        {
            ...
            try
            {
                ...
                // ── Étape 1 : Collecter toutes les données (reader fermé avant toute action) ──
                var surveyIds     = new List<int>();
                var recordDataIds = new List<int>();   //  séparé du reader
                ...
                await using (var cmd = conn.CreateCommand())
                {
                    ...
                    while (await reader.ReadAsync())
                    {
                        ...
                        if (!reader.IsDBNull(1))
                            recordDataIds.Add(reader.GetInt32(1)); // ✅ stocke, ne traite pas
                    }
                    // reader fermé ici (fin du using)
                }

                // ── Étape 2 : Détacher les audios (reader bien fermé) ──
                foreach (var recId in recordDataIds)
                {
                    ...
                    await cmd.ExecuteNonQueryAsync(); // ✅ aucun reader actif
                }

                // ── Étape 3 : Supprimer items et scores en lot ──
                if (surveyIds.Any())
                {
                ...
                // ── Étape 4 : Supprimer les surveys puis la fiche ──
                await using (var cmd = conn.CreateCommand())
                {
                    cmd.Transaction = (System.Data.Common.DbTransaction)tx;
        ...
        //     Amélioration : Is_saved = 1 ajouté sur la requête
        //       des surveys (cohérent avec le reste du code)
        // ─────────────────────────────────────────────────────
        public async Task<AgentReportDto> GetAgentReportAsync(int lsId,int recordDataId = 0)
        {
            ...
            {
                cmd.CommandText = @"
                    SELECT Agent, userName, CreateDate, StartPeriode, EndPeriode
                    FROM Ls WHERE Id = @id";
                AddParam(cmd, "@id", lsId);
                await using var reader = await cmd.ExecuteReaderAsync();
                if (await reader.ReadAsync())
            ...
            await using (var cmd = conn.CreateCommand())
    {
        // ✅ Filtrer par recordDataId si fourni
        cmd.CommandText = @"
        ...
        while (await reader.ReadAsync())
        {
            ...
            // ✅ Utilise la fonction SQL avec filtre recordDataId
double totalScore = Math.Round(
    ...
    2);
        ...
        // ─────────────────────────────────────────────────────
        // : Ajouter un paramètre en une ligne
        //    Remplace les 3 lignes répétitives CreateParameter /
        //    ParameterName / Add par un appel unique
        // ─────────────────────────────────────────────────────
